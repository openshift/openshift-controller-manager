...
func newOperatorTestCommand(ctx context.Context) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "openshift-controller-manager-tests-ext",
		Short: "A binary used to run openshift-controller-manager tests as part of OTE.",
...
func prepareOperatorTestsRegistry() *oteextension.Registry {
	...
	extension := oteextension.NewExtension("openshift", "payload", "openshift-controller-manager")
