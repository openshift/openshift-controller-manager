# OpenShift Controller Manager Tests Extension
========================================================

This repository contains the tests for the OpenShift Controller Manager for OpenShift.
...
| `./openshift-controller-manager-tests-ext list` | Lists all available test cases. |
| `./openshift-controller-manager-tests-ext run-suite <suite-name>` | Runs a test suite. e.g., `openshift/openshift-controller-manager/conformance/parallel` |
| `./openshift-controller-manager-tests-ext run-test <test-name>` | Runs one specific test. |

The tests can be run locally using the `openshift-controller-manager-tests-ext` binary against an OpenShift cluster.
...
3. Run the tests using the `openshift-controller-manager-tests-ext` binary.
...
```shell
./openshift-controller-manager-tests-ext run-suite openshift/openshift-controller-manager/conformance/parallel --junit-path $(ARTIFACT_DIR)/junit_$(shell date +%Y%m%d-%H%M%S).xml
```
...
|------------|-------------|
| `openshift/openshift-controller-manager/conformance/parallel` | Parallel conformance tests |
| `openshift/openshift-controller-manager/conformance/serial` | Serial conformance tests |
| `openshift/openshift-controller-manager/optional/slow` | Optional slow tests |
| `openshift/openshift-controller-manager/all` | All tests |
...
- as: tests-extension
  commands: |
    echo "Build binary openshift-controller-manager-tests-ext"
    make tests-ext-build
    echo "Running ./openshift-controller-manager-tests-ext with sanity test"
    ./openshift-controller-manager-tests-ext run-suite \
      "openshift/openshift-controller-manager/conformance/parallel" \
...
**Note:** Metadata is stored in: `.openshift-tests-extension/openshift-controller-manager.json`

## FAQ

### Why don't we have a Dockerfile for `openshift-controller-manager-tests-ext`?

We do not provide a Dockerfile for `openshift-controller-manager-tests-ext` because building and shipping a
...
The OTE binary (`openshift-controller-manager-tests-ext`) provides:
