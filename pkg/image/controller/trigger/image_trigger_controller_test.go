	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if inst.namespace != "test2" || !reflect.DeepEqual(inst.req, test.req) {
				t.Errorf("For test %s unexpected: %s %s", test.name, inst.namespace, cmp.Diff(test.req, inst.req))
			}
			if inst.req == nil {
...
func TestBuildConfigTriggerIndexer(t *testing.T) {
	...
	if e := scenario_1_buildConfig_strategy_cacheEntry(); !ok || !reflect.DeepEqual(e, actual) {
		t.Fatalf("unexpected: %s", cmp.Diff(e, actual))
	}
...
func verifyEntriesAt(c cache.ThreadSafeStore, entries []interface{}, keys ...string) error {
	for _, key := range keys {
		...
		if e, a := entries, indexed; !reflect.DeepEqual(e, a) {
			return fmt.Errorf("unexpected entry for key %s: %s", key, cmp.Diff(e, a))
		}
	}
	return nil
}
	...
	return &buildv1.BuildConfig{
		...
		Status: buildv1.BuildConfigStatus{
			ImageChangeTriggers: []buildv1.ImageChangeTriggerStatus{
				{},
				{
					From:                 buildv1.ImageStreamTagReference{Name: "stream:2", Namespace: "other"},
					LastTriggeredImageID: "image/result:2",
				},
			},
		},
...
func benchmark_1_buildConfig(r *rand.Rand, identity, maxStreams, maxTags, triggers int32) *buildv1.BuildConfig {
	...
	for i := int32(0); i < (triggers - 1); i++ {
		...
		bc.Status.ImageChangeTriggers = append(bc.Status.ImageChangeTriggers, buildv1.ImageChangeTriggerStatus{
			From: buildv1.ImageStreamTagReference{Name: randomStreamTag(r, maxStreams, maxTags)},
		})
	}
	return bc
}
	...
	for i := 0; i < times; i++ {
		...
		for _, obj := range bcInformer.GetStore().List() {
			if bc, err := updateBuildConfigImages(obj.(*buildv1.BuildConfig), c.tagRetriever); bc != nil || err != nil {
				failures = append(failures, fmt.Sprintf("%s is not fully resolved: %v %s", obj.(*buildv1.BuildConfig).Name, err, cmp.Diff(obj, bc)))
				continue
			}
		}
		if len(failures) == 0 {
