...
type TagImportPolicy struct {
	...
	// ImportMode describes how to import an image manifest. Valid values are SingleManifest and ManifestList.
	// The SingleManifest mode will result in the manifest list itself being discarded, and only one of its
	// sub-manifests being imported, the architecture and os are then chosen in the following order of priority:
	// 1. tag annotations; 2. control plane arch/os; 3. linux/amd64; 4. the first manifest in the list.
	// In ManifestList mode the manifest list itself and all sub-manifests listed in it will be imported.
	// This field is optional. SingleManifest is the default behaviour.
...
const (
	// ImportModeSingleManifest indicates that one of the sub-manifests in the manifest list should be imported, and the
	// manifest list itself discarded. This is the default behaviour.
	ImportModeSingleManifest ImportModeType = "SingleManifest"
	// ImportModeManifestList indicates that the manifest list and all its sub-manifests should be imported.
	ImportModeManifestList ImportModeType = "ManifestList"
)
